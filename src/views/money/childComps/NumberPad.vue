<template>
  <div>
    <notes :value="outputValue" @dateSelect='dateSelect' @textContent='textContent' />
    <div id="number" class="clear-fix">
      ...
      <button class="done" :class="{income:isIncome}" @click="done">完成</button>
  ...
  methods:{
    ...
    deleteValue(){
      if(this.outputValue.length === 1){
        ...
        this.outputValue = this.outputValue.substr(0, this.outputValue.length - 1)
      }
    },
    calcValue(type){
      if(this.saveValue === 0){
